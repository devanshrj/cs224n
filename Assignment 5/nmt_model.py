...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder(X)  # applying encoder LSTM
        enc_hiddens, _ = nn.utils.rnn.pad_packed_sequence(
            enc_hiddens, batch_first=True)  # batch_first changes dimensions

        # step 3
        # concatenating so that(b, 2*h)
        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        init_decoder_hidden = self.h_projection(
            last_hidden)            # applying linear layer
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # step 2
        Y = self.model_embeddings_target(target_padded)         # applying nn.Embeddings
        
        # step 3
        Y_split = torch.split(Y, split_size_or_sections=1, dim=0)
